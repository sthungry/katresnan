...
function SectionHeader({ label, title }: { label: string; title: React.ReactNode }) {
  return (
    <div className='text-center mb-8'>
      <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.55rem', letterSpacing: '0.5em', textTransform: 'uppercase', color: C.accentDim, display: 'block', marginBottom: 8 }}>{label}</span>
      <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(1.5rem,5vw,2rem)', fontWeight: 600, color: C.text, lineHeight: 1.2, margin: '0 0 4px' }}>{title}</h2>
      <LineDivider />
    </div>
  )
}
    ...
    <section style={{ background: 'transparent' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        <div className='text-center px-6 pt-16 pb-6'>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.55rem', letterSpacing: '0.5em', textTransform: 'uppercase', color: C.accentDim, marginBottom: 10 }}>The Wedding of</p>
          <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontWeight: 700, fontSize: 'clamp(1.6rem,5vw,2.2rem)', color: C.text, lineHeight: 1.1, textShadow: '0 2px 20px rgba(0,0,0,0.4)' }}>
        ...
        {/* Photo Slideshow */}
        <div className='relative overflow-hidden rounded-xl' style={{ aspectRatio: '3/4', maxHeight: '60vh', margin: '0 16px 24px' }}>
          {fotos.map((url, i) => (
          ...
          {fotos.length > 1 && (
            <div className='absolute top-3 right-3 z-10'>
              <span style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.5rem', letterSpacing: '0.15em', color: 'rgba(255,255,255,0.4)', background: 'rgba(0,0,0,0.3)', padding: '3px 8px', borderRadius: 16, backdropFilter: 'blur(4px)' }}>
  ...
  return (
    <section className='relative overflow-hidden' style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', background: 'transparent' }}>
      ...
      <div className='relative z-10 text-center px-8'>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.6rem', letterSpacing: '0.6em', textTransform: 'uppercase', color: C.muted, marginBottom: 20 }}>The Wedding of</p>
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontWeight: 700, fontSize: 'clamp(2.6rem,11vw,4.2rem)', color: C.text, lineHeight: 1, marginBottom: 0, textShadow: '0 2px 20px rgba(0,0,0,0.5)' }}>
          {w.pria_nama_panggilan || 'Nama'}
        </h1>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 'clamp(1.3rem,4.5vw,2rem)', color: C.muted, lineHeight: 1, margin: '8px 0' }}>&amp;</p>
        <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontWeight: 700, fontSize: 'clamp(2.6rem,11vw,4.2rem)', color: C.text, lineHeight: 1, marginBottom: 16, textShadow: '0 2px 20px rgba(0,0,0,0.5)' }}>
        ...
        {w.resepsi_tanggal && <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.65rem', letterSpacing: '0.35em', textTransform: 'uppercase', color: C.subtle, marginBottom: 32 }}>{fmtShort(w.resepsi_tanggal)}</p>}
        <button onClick={onOpen} className='transition-all active:scale-95 hover:scale-105'
          style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.65rem', letterSpacing: '0.3em', textTransform: 'uppercase', padding: '12px 36px', background: 'rgba(255,255,255,0.08)', border: `1px solid rgba(255,255,255,0.3)`, color: '#fff', cursor: 'pointer', borderRadius: 0, backdropFilter: 'blur(4px)', transition: 'all 0.3s ease' }}>
          Buka Undangan
        </button>
      </div>
    </section>
      ...
      <div className='relative z-10 px-6 py-20 text-center' style={{ maxWidth: 560, margin: '0 auto' }}>
        {isCustom ? (
          <>
            ...
            <blockquote style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 'clamp(0.95rem,2.5vw,1.15rem)', color: C.text, lineHeight: 2, maxWidth: 480, margin: '0 auto 20px', borderLeft: `2px solid ${C.accent}`, paddingLeft: 20, textAlign: 'left' }}>
          ...
          <>
            ...
            <blockquote style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 'clamp(0.95rem,2.5vw,1.15rem)', color: C.text, lineHeight: 2, maxWidth: 480, margin: '0 auto 16px', borderLeft: `2px solid ${C.accent}`, paddingLeft: 20, textAlign: 'left' }}>
              {preset.text}
            </blockquote>
            <cite style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.65rem', letterSpacing: '0.35em', textTransform: 'uppercase', color: C.accentDim, fontStyle: 'normal', display: 'block', marginBottom: 20 }}>
              {preset.source}
            </cite>
  ...
  const Card = ({ type }: { type: 'pria' | 'wanita' }) => {
    ...
    return (
      <div className='text-center'>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.55rem', letterSpacing: '0.4em', textTransform: 'uppercase', color: C.accentDim, marginBottom: 12 }}>{p ? 'The Groom' : 'The Bride'}</p>
        <div className='relative mx-auto mb-5' style={{ width: 140, height: 140 }}>
          <div style={{ position: 'absolute', inset: '-4px', border: `1px solid ${C.border}`, borderRadius: '50%' }} />
          <div className='w-full h-full rounded-full overflow-hidden' style={{ border: `2px solid ${C.accent}`, boxShadow: `0 8px 32px rgba(0,0,0,0.4)` }}>
            {foto ? <img src={foto} alt={nama || ''} className='w-full h-full object-cover' style={{ objectPosition: 'center top' }} />
              : <div className='w-full h-full flex items-center justify-center text-4xl' style={{ background: C.card }}>{p ? '🤵' : '👰'}</div>}
          </div>
        </div>
        <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: '1.25rem', color: C.accent, fontWeight: 600, marginBottom: 4, lineHeight: 1.3 }}>
          {nama}{gelar ? ` ${gelar}` : ''}
        </h3>
      ...
      <div className='relative z-10 px-5 py-20 text-center' style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '28px 16px', alignItems: 'start' }}>
          ...
          <div className='flex flex-col items-center' style={{ paddingTop: 32 }}>
            ...
            <span style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: '1.7rem', color: C.accent, padding: '4px 0' }}>&amp;</span>
    ...
    return (
      <div className='rounded-lg overflow-hidden' style={{ background: C.card, border: `1px solid ${C.border}` }}>
        ...
        <div className='p-5 text-center'>
          <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.55rem', letterSpacing: '0.4em', textTransform: 'uppercase', color: C.accentDim, display: 'block', marginBottom: 4 }}>{isA ? '01' : '02'}</span>
          <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontWeight: 600, fontSize: '1.1rem', color: C.accent, marginBottom: 10 }}>{isA ? 'Akad Nikah' : 'Resepsi Pernikahan'}</h3>
          ...
          {mp && <a href={mp} target='_blank' rel='noopener noreferrer' className='inline-flex items-center gap-1.5 mt-4 transition-all active:scale-95' style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.62rem', letterSpacing: '0.08em', padding: '9px 20px', borderRadius: 2, background: 'transparent', border: `1px solid ${C.accent}`, color: C.accent, textDecoration: 'none' }}>📍 Lihat Lokasi</a>}
        </div>
      </div>
    )
  }
  return (
    <section id='acara' className='relative overflow-hidden' style={{ background: 'transparent' }}>
      ...
      <div className='relative z-10 px-5 py-20 text-center' style={{ maxWidth: 560, margin: '0 auto' }}>
            ...
            {(['d', 'h', 'm', 's'] as const).map(k => {
              ...
              return (
                <div key={k} className='flex flex-col items-center' style={{ background: C.card, border: `1px solid ${C.border}`, width: 68, paddingTop: 14, paddingBottom: 14 }}>
                  <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.9rem', fontWeight: 700, color: C.accent, lineHeight: 1 }}>{String(val).padStart(2, '0')}</span>
                  <span style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.5rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: C.subtle, marginTop: 5 }}>{lbl}</span>
                </div>
              )
            })}
          </div>
        ) : <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: '1.3rem', color: C.accent, margin: '24px 0' }}>🎉 Hari Bahagia Telah Tiba!</p>}
        ...
        {target && <a href={gcalUrl(w)} target='_blank' rel='noopener noreferrer' className='inline-flex items-center gap-2 transition-all active:scale-95 mt-2' style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.62rem', letterSpacing: '0.08em', padding: '10px 22px', borderRadius: 2, border: `1.5px solid ${C.accent}`, color: C.accent, background: 'transparent', textDecoration: 'none' }}>📅 Tambah ke Kalender</a>}
      </div>
    </section>
  )
}
...
function SectionRSVP({ w }: { w: WeddingData }) {
  ...
  const iS: React.CSSProperties = { width: '100%', padding: '12px 16px', border: `1px solid ${C.border}`, borderRadius: 2, fontFamily: "'Lato',sans-serif", fontSize: '0.82rem', color: C.text, background: C.card, outline: 'none', boxSizing: 'border-box' }
  return (
      ...
      <div className='relative z-10 px-5 py-20 text-center' style={{ maxWidth: 520, margin: '0 auto' }}>
        ...
        {!ok ? (
          <div className='text-left space-y-4'>
            <div><label style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: C.accentDim, display: 'block', marginBottom: 7 }}>Nama</label><input value={form.nama} onChange={e => setForm(f => ({ ...f, nama: e.target.value }))} placeholder='Nama lengkap Anda' style={iS} /></div>
            <div><label style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: C.accentDim, display: 'block', marginBottom: 7 }}>Kehadiran</label>
              <div className='flex gap-2'>{[['hadir', '✅ Hadir'], ['tidak', '❌ Tidak'], ['ragu', '🤔 Mungkin']].map(([v, l]) => (
                <button key={v} onClick={() => setForm(f => ({ ...f, hadir: v }))} className='flex-1 transition-all active:scale-95'
                  style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.7rem', padding: '10px 4px', borderRadius: 2, border: `1px solid ${form.hadir === v ? C.accent : C.border}`, background: form.hadir === v ? `rgba(255,255,255,0.12)` : C.card, color: form.hadir === v ? C.accent : C.muted, cursor: 'pointer', fontWeight: form.hadir === v ? 700 : 400 }}>{l}</button>
              ))}</div></div>
            <div><label style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: C.accentDim, display: 'block', marginBottom: 7 }}>Ucapan &amp; Doa</label><textarea value={form.ucapan} onChange={e => setForm(f => ({ ...f, ucapan: e.target.value }))} placeholder='Tulis ucapan terbaikmu...' rows={3} style={{ ...iS, resize: 'none' }} /></div>
            <button onClick={send} disabled={busy || !form.nama || !form.ucapan} className='w-full transition-all active:scale-95 disabled:opacity-50'
              style={{ fontFamily: "'Lato',sans-serif", fontWeight: 700, fontSize: '0.72rem', letterSpacing: '0.18em', textTransform: 'uppercase', padding: '14px', borderRadius: 2, background: `linear-gradient(135deg,${C.accent},${C.accentSoft})`, color: C.bg, border: 'none', cursor: 'pointer', boxShadow: `0 6px 20px rgba(255,255,255,0.25)` }}>
              {busy ? 'Mengirim...' : 'Kirim Ucapan 💌'}
            </button>
          </div>
        ) : <div className='py-8 rounded-lg' style={{ background: `rgba(255,255,255,0.08)`, border: `1px solid ${C.border}` }}><p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: '1rem', color: C.accent }}>Terima kasih atas ucapan dan doamu! ✨</p></div>}
        <div className='mt-8'>
          <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: '0.85rem', color: C.accentDim, marginBottom: 12 }}>— Ucapan &amp; Doa —</p>
          <div className='space-y-3 text-left' style={{ maxHeight: 300, overflowY: 'auto' }}>
            {loading ? <p className='text-center text-sm py-4' style={{ color: C.subtle }}>Memuat...</p>
              : list.length === 0 ? <p className='text-center text-sm py-4' style={{ color: C.subtle }}>Jadilah yang pertama memberi ucapan ✨</p>
                : list.map(u => (<div key={u.id} className='rounded-lg px-4 py-3' style={{ background: C.card, borderLeft: `2px solid ${C.accent}`, border: `1px solid ${C.borderLight}` }}>
                  <div className='flex justify-between mb-1'><p style={{ fontFamily: "'Lato',sans-serif", fontWeight: 700, fontSize: '0.78rem', color: C.accentSoft }}>{u.nama}</p><span>{u.hadir === 'hadir' ? '✅' : u.hadir === 'tidak' ? '❌' : '🤔'}</span></div>
            ...
            const foto = w.foto_urls?.[s.fi]; return (
              <div key={i} className='flex items-center gap-5' style={{ flexDirection: i % 2 === 1 ? 'row-reverse' : 'row' }}>
                <div className='flex-1 text-left'>
                  {s.yr && <span style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.55rem', letterSpacing: '0.35em', textTransform: 'uppercase', color: C.accentDim, display: 'block', marginBottom: 3 }}>{String(s.yr)}</span>}
                  <h4 style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontWeight: 600, fontSize: '1.05rem', color: C.accent, marginBottom: 5 }}>{s.title}</h4>
                ...
                {foto && <div className='flex-shrink-0 overflow-hidden rounded-lg' style={{ width: 65, height: 65, border: `1px solid ${C.border}` }}><img src={foto} alt='' className='w-full h-full object-cover' style={{ objectPosition: 'center top' }} /></div>}
              </div>
            )
          })}
        </div>
      ...
      <div className='relative z-10 px-5 py-20 text-center' style={{ maxWidth: 540, margin: '0 auto' }}>
        ...
        <div className='space-y-5 text-left'>
          {(['pria', 'wanita', 'bersama'] as const).map(cat => {
            ...
            return (<div key={cat}>
              <p style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.58rem', letterSpacing: '0.35em', textTransform: 'uppercase', color: C.accentDim, fontWeight: 700, marginBottom: 8 }}>{lbl}</p>
              <div className='space-y-3'>{lst.map(r => (<div key={r.id} className='rounded-lg p-4' style={{ background: C.card, border: `1px solid ${C.borderLight}` }}>
                <div className='flex items-center justify-between gap-3'>
                  <div className='min-w-0'>
                    <p style={{ fontFamily: "'Lato',sans-serif", fontWeight: 700, fontSize: '0.68rem', color: C.accentDim, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{r.bank}</p>
                    <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.3rem', color: C.accent, letterSpacing: '0.04em', marginTop: 2, marginBottom: 2 }}>{r.nomor}</p>
                    <p style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.68rem', color: C.subtle }}>a.n. {r.nama}</p>
                  </div>
                  <button onClick={() => copy(r.nomor, r.id)} className='flex-shrink-0 transition-all active:scale-90'
                    style={{
                      fontFamily: "'Lato',sans-serif", fontSize: '0.6rem', fontWeight: 700, padding: '8px 14px', borderRadius: 2, cursor: 'pointer', minWidth: 68,
        ...
        {w.alamat_kado && (<div className='mt-4 rounded-lg p-4 text-left' style={{ background: C.card, border: `1px solid ${C.borderLight}` }}>
          <p style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.58rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: C.accentDim, marginBottom: 6 }}>📦 Alamat Kado Fisik</p>
  ...
  return (
    <section id='footer' className='relative overflow-hidden' style={{ background: 'transparent' }}>
      ...
      <div className='relative z-10 text-center px-6 py-20' style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontWeight: 700, fontSize: 'clamp(2rem,8vw,3.2rem)', color: C.accent, lineHeight: 1.1, margin: '20px 0 8px' }}>
          {w.pria_nama_panggilan} &amp; {w.wanita_nama_panggilan}
        </h2>
        <LineDivider width={50} />
        {w.resepsi_tanggal && <p style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.68rem', letterSpacing: '0.25em', color: C.subtle, marginTop: 8 }}>{fmtShort(w.resepsi_tanggal)}</p>}
        {w.hashtag_instagram && <p style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.68rem', color: C.subtle, marginTop: 10 }}>{w.hashtag_instagram}</p>}
      </div>
      {/* Copyright */}
      <div style={{ background: 'rgba(255,255,255,0.08)', padding: '12px 16px', textAlign: 'center', borderTop: `1px solid ${C.borderLight}` }}>
        <p style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.58rem', color: C.subtle, letterSpacing: '0.12em' }}>
  ...
  return (
    <section id='video' className='relative overflow-hidden' style={{ background: 'transparent' }}>
      <div className='relative z-10 py-20' style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        <div className='mx-4 relative overflow-hidden rounded-xl' style={{ aspectRatio: '16/9' }}>
          <iframe
        ...
        <p className='text-center mt-4 px-5' style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.75rem', color: C.muted, lineHeight: 1.7 }}>
...
function DesktopSlideshow({ w }: { w: WeddingData }) {
  ...
  if (!fotos.length) return (
    <div className='w-full h-full flex items-center justify-center' style={{ background: `linear-gradient(135deg,${C.bg},#1a1a2e)` }}>
      <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '2rem', color: C.accentDim, fontStyle: 'italic', opacity: 0.3 }}>Gallery</p>
    </div>
  )
  return (
    <div className='relative w-full h-full overflow-hidden' style={{ background: C.bg }}>
      ...
      <div className='absolute bottom-0 left-0 right-0 px-12 pb-12 z-10'>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.58rem', letterSpacing: '0.55em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', marginBottom: 8 }}>Undangan Pernikahan</p>
        ...
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 'clamp(1.4rem,2.2vw,2rem)', color: C.accent, lineHeight: 1, margin: '4px 0' }}>&amp;</p>
        ...
        {w.resepsi_tanggal && <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '0.7rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)' }}>{fmtShort(w.resepsi_tanggal)}</p>}
      </div>
      {fotos.length > 1 && (
        <>
          ...
          <div className='absolute top-7 right-8 z-20'>
            <span style={{ fontFamily: "'Lato',sans-serif", fontSize: '0.58rem', letterSpacing: '0.18em', color: 'rgba(255,255,255,0.3)' }}>{String(cur + 1).padStart(2, '0')} / {String(fotos.length).padStart(2, '0')}</span>
          </div>
        </>
      )}
    </div>
