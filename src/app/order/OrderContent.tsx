    ...
    <div className="w-full max-w-md mx-auto">
      <div className="relative">
        <input
          ...
          maxLength={8}
          className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-primary-400 dark:focus:border-primary-500 rounded-2xl px-5 py-4 pr-36 text-sm text-slate-800 dark:text-slate-200 outline-none transition-colors placeholder:text-slate-400 font-mono tracking-widest uppercase"
        />
        <button
          onClick={() => input && onSearch(input)}
          disabled={!input || loading}
          className="absolute right-2 top-2 bottom-2 bg-slate-800 dark:bg-slate-700 hover:bg-slate-700 dark:hover:bg-slate-600 disabled:opacity-50 text-white font-bold px-5 rounded-xl transition-colors text-sm"
        >
          {loading ? (
...
function OrderCard({ order }: { order: Order }) {
  ...
  return (
    <div className="w-full max-w-lg mx-auto space-y-4" style={{ opacity: 0, animation: 'fadeUp 0.5s ease forwards' }}>
      ...
      {/* Status Banner */}
      <div className={`rounded-2xl border p-6 ${cfg.bg} border-slate-200/50 dark:border-slate-700/50`}>
        <div className="flex items-start gap-4">
          <div className={cfg.color}>{cfg.icon}</div>
          <div className="flex-1 min-w-0">
            <p className={`font-serif font-bold text-xl ${cfg.color}`}>{cfg.label}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">{cfg.desc}</p>
          </div>
        </div>
                ...
                <div key={s.step} className="flex items-center flex-1 last:flex-none">
                  <div className="flex flex-col items-center gap-1">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all border-2 ${
                      done   ? 'bg-slate-800 dark:bg-slate-600 text-white border-slate-800 dark:border-slate-600'
                      : active ? `${cfg.bg} ${cfg.color} border-current`
                      : 'bg-white dark:bg-slate-900 text-slate-300 border-slate-200 dark:border-slate-700'
                    }`}>
                      {done ? (
                    ...
                    <span className={`text-[9px] font-medium text-center leading-tight w-14 ${
                      active ? cfg.color : done ? 'text-slate-700 dark:text-slate-300' : 'text-slate-300 dark:text-slate-600'
                    }`}>{s.label}</span>
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={`flex-1 h-0.5 mb-4 mx-1 transition-all ${done ? 'bg-slate-800 dark:bg-slate-600' : 'bg-slate-200 dark:bg-slate-700'}`}/>
                  )}
                </div>
              )
            })}
        ...
        <div className="p-5 space-y-3.5 text-sm">
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-3.5 space-y-3.5">
            ...
            <div className="flex justify-between items-center">
              <span className="font-bold text-slate-800 dark:text-surface-200">Total Bayar</span>
              <span className="font-serif font-bold text-xl text-slate-800 dark:text-primary-400">{formatRp(order.total)}</span>
            </div>
          </div>
          {order.created_at && (
      ...
      {order.status === 'pending' && (
        <Link href={`/checkout?paket=${order.package_id}`}
          className="flex items-center justify-center gap-2 w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-4 rounded-2xl text-center transition-colors shadow-lg">
      ...
      {order.status === 'selesai' && (
        <div className="bg-surface-100 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 text-center">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-8 h-8 mx-auto mb-2 text-primary-500">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
          </svg>
          <p className="font-bold text-slate-800 dark:text-surface-200 mb-1">Undangan sudah aktif!</p>
          <p className="text-xs text-slate-500 dark:text-slate-400">Link undangan sudah dikirim ke WhatsApp & email kamu.</p>
        </div>
      )}
      {order.status === 'dibatalkan' && (
        <Link href="/#harga"
          className="flex items-center justify-center gap-2 w-full bg-tertiary-500 hover:bg-tertiary-600 text-white font-bold py-4 rounded-2xl text-center transition-colors">
      ...
      <p className="text-center text-xs text-slate-400 dark:text-slate-500 pt-2">
        ...
        <a href="https://wa.me/6285150000715" target="_blank" rel="noopener noreferrer"
          className="text-slate-700 dark:text-slate-300 underline underline-offset-2 decoration-primary-300 dark:decoration-primary-600 font-medium hover:text-slate-900 dark:hover:text-white transition-colors">
...
function Row({ label, value, bold, accent }: { label: string; value: string; bold?: boolean; accent?: boolean }) {
  return (
    <div className="flex justify-between items-center gap-4">
      ...
      <span className={`text-right truncate ${bold ? 'font-bold text-slate-800 dark:text-surface-200' : accent ? 'text-primary-600 dark:text-primary-400 font-medium' : 'text-slate-700 dark:text-slate-300'}`}>{value}</span>
    </div>
  )
}
...
function OrderInner() {
  ...
  return (
    <div className="min-h-screen bg-surface-100 dark:bg-slate-950 transition-colors duration-500">
      ...
      {/* Header */}
      <div className="glass border-b border-slate-100 dark:border-slate-800 px-4 py-4 sticky top-0 z-40">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-full px-3 py-1.5 flex items-center gap-1.5">
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-slate-100 to-primary-100 dark:from-slate-800 dark:to-slate-700 mb-5 shadow-sm">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-7 h-7 text-slate-600 dark:text-slate-300">
              <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
            </svg>
          </div>
          <h1 className="font-serif text-3xl md:text-4xl font-bold text-slate-800 dark:text-surface-200 mb-3">
            Cek Status Pesanan
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm max-w-sm mx-auto leading-relaxed">
            Masukkan <strong className="text-slate-700 dark:text-slate-300">8 karakter Order ID</strong> yang dikirim ke email kamu saat checkout.
          </p>
        </div>
        ...
        {loading && (
          <div className="text-center py-12">
            <div className="inline-flex flex-col items-center gap-3">
              <div className="w-12 h-12 rounded-full border-4 border-slate-200 dark:border-slate-700 border-t-slate-800 dark:border-t-slate-300 animate-spin"/>
              <p className="text-slate-500 dark:text-slate-400 text-sm">Mencari pesanan...</p>
            </div>
          </div>
        )}
        ...
        {!loading && !order && !error && !searched && (
          <div className="grid grid-cols-3 gap-3 mt-6">
            {[
              ...
              <div key={item.title} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 text-center">
                <div className="text-slate-400 dark:text-slate-500 flex justify-center mb-2">{item.icon}</div>
                <p className="font-serif font-bold text-xs text-slate-800 dark:text-surface-200 mb-1">{item.title}</p>
