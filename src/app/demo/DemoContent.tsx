...
function TemplateSkeleton() {
  return (
    <div className="rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 animate-pulse">
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="flex gap-1.5">
          ...
          <div className="flex-1 h-8 bg-slate-200 dark:bg-slate-700 rounded-lg"/>
        </div>
      </div>
    </div>
  )
...
function TemplateCard({
  ...
  return (
    <div className={`relative group rounded-2xl overflow-hidden border transition-all duration-200 bg-white dark:bg-slate-900 flex flex-col
      ${selected
        ? 'border-slate-800 dark:border-primary-500 shadow-xl ring-2 ring-slate-800/20 dark:ring-primary-500/20 scale-[1.02]'
        : disabled
          ? 'border-slate-100 dark:border-slate-800 opacity-40 cursor-not-allowed'
          : 'border-slate-100 dark:border-slate-800 hover:border-primary-300 dark:hover:border-primary-600 hover:shadow-lg cursor-pointer'
      }
    `}>
      <div className="relative aspect-[9/16] overflow-hidden bg-surface-100 dark:bg-slate-950" onClick={() => !disabled && onSelect()}>
        <Image
        ...
        />
        <div className={`absolute inset-0 transition-opacity ${selected ? 'bg-slate-800/15 dark:bg-primary-500/10' : 'bg-black/0 group-hover:bg-black/5'}`}/>
        {selected && (
          <div className="absolute top-2.5 right-2.5 w-8 h-8 bg-slate-800 dark:bg-primary-500 rounded-full flex items-center justify-center text-white dark:text-slate-950 shadow-lg">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
          </div>
        )}
        {tpl.is_popular && (
          <div className="absolute top-2.5 left-2.5 flex items-center gap-1 bg-primary-500 text-white text-[9px] font-bold px-2.5 py-1 rounded-full shadow">
        ...
        {tpl.category && (
          <div className="absolute bottom-2.5 left-2.5 bg-slate-900/50 text-white text-[9px] font-semibold px-2 py-0.5 rounded-full backdrop-blur-sm">
      ...
      <div className="p-3 flex flex-col gap-2">
        <div>
          <p className="font-serif font-bold text-sm text-slate-800 dark:text-surface-200">{tpl.name}</p>
          <p className="text-[10px] text-slate-500 dark:text-slate-400 leading-tight mt-0.5">{tpl.description}</p>
        </div>
        <div className="flex gap-1.5">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all flex items-center justify-center gap-1 ${
              selected
                ? 'bg-slate-800 dark:bg-primary-500 text-white dark:text-slate-950'
                : disabled
                  ? 'bg-slate-100 dark:bg-slate-800 text-slate-300 dark:text-slate-600 cursor-not-allowed'
                  : 'bg-surface-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'
            }`}
          >
            {selected ? (
          ...
          <a
            ...
            className="flex-1 py-2 rounded-lg text-[10px] font-bold bg-slate-800 dark:bg-slate-700 text-white flex items-center justify-center gap-1 hover:bg-slate-700 dark:hover:bg-slate-600 transition-colors"
          >
...
function DemoInner() {
  ...
  return (
    <div className="min-h-screen bg-surface-100 dark:bg-slate-950 transition-colors duration-500">
      ...
      {/* Header */}
      <div className="sticky top-0 z-40 glass border-b border-slate-100 dark:border-slate-800 px-4 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link href="/#harga" className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white text-sm font-medium transition-colors">
          ...
          {pkg && (
            <div className="flex items-center gap-2 glass rounded-full px-4 py-1.5">
              <span className="font-serif font-bold text-sm text-slate-800 dark:text-surface-200">Paket {pkg.name}</span>
              <span className="hidden sm:inline text-xs text-slate-500 dark:text-slate-400">— max {limit} tema</span>
              <span className="hidden sm:inline text-xs font-bold text-slate-700 dark:text-primary-400">
                {formatRp(pkg.price)}
              </span>
            </div>
          )}
      ...
      <div className="max-w-6xl mx-auto px-4 py-8" id="template-grid">
        ...
        <div className="text-center mb-8">
          <h1 className="font-serif text-3xl md:text-4xl font-bold text-slate-800 dark:text-surface-200 mb-2">
            Pilih Template{' '}
            {pkg && <span className="gradient-text">Paket {pkg.name}</span>}
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">
            Klik <strong className="text-slate-700 dark:text-slate-300">Preview</strong> untuk melihat demo live, lalu <strong className="text-slate-700 dark:text-slate-300">+ Pilih</strong> untuk memilih.
          </p>

          {pkg?.id === 'silver' && (
            <div className="inline-flex items-center gap-2 bg-primary-50 dark:bg-primary-900/20 border border-primary-200/60 dark:border-primary-700/40 text-primary-700 dark:text-primary-400 rounded-full px-4 py-2 text-xs font-semibold mb-3">
          ...
          <div className={`inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-semibold border transition-all ${
            limitReached
              ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-300/60 dark:border-primary-700/40 text-primary-600 dark:text-primary-400'
              : showError
            ...
            {!limitReached && (
              <span className="text-slate-400 dark:text-slate-500 text-xs font-normal">({limit - selected.length} lagi)</span>
            )}
          </div>
        </div>
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
                filterCat === 'semua'
                  ...
                  : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-primary-300'
              }`}
            >
              Semua
            </button>
            ...
            : filtered.length === 0
              ? (
                <div className="col-span-full text-center py-16">
                  <svg className="w-10 h-10 mx-auto mb-3 text-slate-300 dark:text-slate-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                  <p className="text-slate-500 dark:text-slate-400">Tidak ada template di kategori ini.</p>
                </div>
              )
              : filtered.map(tpl => (
                  <TemplateCard
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40">
        {!canProceed && (
          <div className="bg-surface-100/95 dark:bg-slate-950/95 backdrop-blur border-t border-slate-100 dark:border-slate-800 px-4 py-3">
        ...
        {canProceed && (
          <div className="glass border-t border-slate-100 dark:border-slate-800 px-4 py-4 shadow-2xl">
            <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
              <div className="min-w-0">
                <p className="text-sm font-bold text-slate-800 dark:text-surface-200 truncate">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                {pkg && (
                  <div className="text-right hidden sm:block">
                    ...
                    <p className="font-serif font-bold text-slate-800 dark:text-primary-400">
                      {formatRp(pkg.price)}
                    </p>
                  </div>
                )}
...
export default function DemoContent() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-surface-100 dark:bg-slate-950">
        <div className="text-center">
          <div className="w-10 h-10 rounded-full border-4 border-slate-200 dark:border-slate-700 border-t-slate-800 dark:border-t-slate-300 animate-spin mx-auto mb-3"/>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Memuat template...</p>
        </div>
      </div>
    }>
      <DemoInner />
