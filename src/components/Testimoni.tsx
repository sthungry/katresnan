  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: count }).map((_, i) => (
        <svg key={i} className="w-4 h-4 text-primary-500 fill-primary-500" viewBox="0 0 20 20">
...
function QuoteIcon() {
  return (
    <svg className="w-8 h-8 text-primary-200 dark:text-slate-700" viewBox="0 0 24 24" fill="currentColor">
...
export default function Testimoni() {
  ...
  return (
    <section id="testimoni" className="py-24 px-6 bg-gradient-to-b from-surface-100 to-white dark:from-slate-950 dark:to-slate-900 transition-colors" ref={sectionRef}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-5 space-y-5">
          {testimonials.map((t, i) => (
            <div key={t.name}
              className="animate-on-scroll card-hover bg-white dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 break-inside-avoid transition-colors"
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-200 to-primary-100 dark:from-slate-700 dark:to-slate-800 flex items-center justify-center flex-shrink-0">
                  <span className="text-slate-600 dark:text-slate-300 text-sm font-serif font-bold">
                    {t.name.charAt(0)}
                  </span>
                </div>
                <div>
                  <p className="text-slate-800 dark:text-surface-200 text-sm font-semibold">{t.name}</p>
                  <p className="text-slate-400 dark:text-slate-500 text-xs">{t.role}</p>
                </div>
              </div>
            </div>
          ))}
