    ...
    <>
      ...
      >
          ...
          <nav className="flex items-center gap-8">
            {navLinks.map(l => (
              <a key={l.label} href={l.href}
                className="text-[13px] font-medium tracking-wide uppercase text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors duration-200">
                {l.label}
              </a>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setMenuOpen(o => !o)} aria-label="Toggle menu"
              ...
              <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2"
                viewBox="0 0 24 24" className="text-slate-700 dark:text-slate-300">
                {menuOpen
        ...
        <div className="lg:hidden overflow-hidden transition-all duration-300"
          ...
          <div className="bg-surface-100 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 px-5 py-4">
            <div className="flex flex-col gap-1 mb-4">
              {navLinks.map((l, i) => (
                <a key={l.label} href={l.href}
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center px-4 py-3 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                  {l.label}
                </a>
              ))}
            </div>
