...
function ChevronIcon({ open }: { open: boolean }) {
  return (
    <svg
      className={`w-5 h-5 text-primary-500 dark:text-primary-400 flex-shrink-0 transition-transform duration-300 ${open ? 'rotate-180' : ''}`}
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden transition-all duration-300 ${
      open
        ? 'border-primary-200/60 dark:border-slate-600 bg-white dark:bg-slate-900/60'
        : 'border-slate-100 dark:border-slate-800 bg-surface-50/50 dark:bg-slate-900/20'
    }`}>
      <button className="w-full flex items-center justify-between gap-4 px-6 py-5 text-left hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors duration-200"
        onClick={() => setOpen(!open)}>
        <span className="text-slate-800 dark:text-surface-200 font-medium text-sm">{q}</span>
      ...
      >
        <div className="px-6 pb-5 text-slate-500 dark:text-slate-400 text-sm leading-relaxed border-t border-slate-100/60 dark:border-slate-700/60 pt-4">
          {a}
        </div>
      </div>
    </div>
...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-24 px-6 bg-white dark:bg-slate-900 transition-colors" ref={sectionRef}>
